...
printf("\nDecrypting message (key = %i): %s\n", key, str);

/* Loops through the message until last index reached */
for (i = 0; i < strlen(str); i++)
    {
        /* Decrypts message if character is lowercase and between a and z */
        if (islower(i) == 0 && str[i] >= 'a' && str[i] <= 'z')
        {
            str[i] = ((str[i] - key) - 'a') % 26 + 'a';
        }
        /* Decrypts message if character is uppercase and between A and Z */
